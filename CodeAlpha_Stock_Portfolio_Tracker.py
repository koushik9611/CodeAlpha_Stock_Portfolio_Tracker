...
API_KEY = ' 71DQOQXNGBX3DZLK'  # Replace this with your actual API key
...
def get_stock_data(symbol):
    ...
    if 'Global Quote' in data:
        ...
        return None
class Portfolio:                #we make a class name is Portfolio
    def __init__(self):
        self.stocks = {}

    def add_stock(self, symbol, quantity):          #add the stocks qty.   
        if symbol in self.stocks:
            self.stocks[symbol] += quantity
        else:
            self.stocks[symbol] = quantity

    def remove_stock(self, symbol, quantity):          #remove stocks qty.
        if symbol in self.stocks:
            self.stocks[symbol] -= quantity
